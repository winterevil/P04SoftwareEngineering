...
class TemperatureSensor {
    ...
    public float getTemperature(SensorData.HealthState state) {
        switch (state) {
            case NORMAL:
                return 36.0f + random.nextFloat() * 1.5f; // 36.0 to 37.5
            case ABNORMAL:
                return random.nextBoolean() ? 35.0f + random.nextFloat() * 1.0f : 37.5f + random.nextFloat() * 1.5f; // 35.0 to 36.0 or 37.5 to 39.0
            case CRITICAL:
                return random.nextBoolean() ? 34.0f + random.nextFloat() * 1.0f : 39.0f + random.nextFloat() * 2.0f; // 34.0 to 35.0 or 39.0 to 41.0
            default:
...
class BloodPressureSensor {
    ...
    public float[] getBloodPressure(SensorData.HealthState state) {
        switch (state) {
            case NORMAL:
                return new float[]{90 + random.nextFloat() * 30, 60 + random.nextFloat() * 20}; // 90-120 systolic, 60-80 diastolic
            case ABNORMAL:
                return new float[]{
...
class HeartRateSensor {
    ...
    public int getHeartRate(SensorData.HealthState state) {
        switch (state) {
            ...
            case ABNORMAL:
                return random.nextBoolean() ? 50 + random.nextInt(10) : 100 + random.nextInt(31); // 50-60 or 100-130 bpm
            case CRITICAL:
                return random.nextBoolean() ? 40 + random.nextInt(10) : 130 + random.nextInt(21); // 40-50 or 130-150 bpm
            default:
                throw new IllegalStateException("Unexpected value: " + state);
        }
    }
